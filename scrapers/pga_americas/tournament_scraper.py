"""
PGA Tour Americas Tournament Scraper
=====================================

Scrapes PGA Tour Americas tournament schedules and results using the PGA Tour GraphQL API.
...
class PGAAmericasTournamentScraper(BasePGAEcosystemScraper):
    """Scrapes PGA Tour Americas tournament schedules and results."""
    ...
    def scrape(self, year: Optional[int] = None, **kwargs) -> Dict[str, Any]:
        """
        Scrape PGA Tour Americas tournaments for a given year.
        ...
        self.logger.info(f'Starting PGA Tour Americas tournament scrape for {year}')
    ...
    def _process_tournament(self, data: Dict, year: int) -> Optional[int]:
        ...
        with self.db.get_session() as session:
            league = session.query(League).filter_by(league_code='PGAAMERICAS').first()
            if not league:
                return None
            ...
            if tournament:
                ...
                tournament = Tournament(
                    ...
                    pga_americas_tournament_id=data.get('tournament_id', '')
                )
    ...
    def _save_result(self, session, tournament, result, tournament_status='COMPLETED'):
        ...
        # Find player by PGA Americas ID
        player = session.query(Player).filter_by(pga_americas_id=tid).first()
        if not player:
        ...
        if not player:
            if not first_name or not last_name:
                return
            player = Player(first_name=first_name, last_name=last_name, pga_americas_id=tid)
            session.add(player)
            session.flush()
...
def scrape_pga_americas_tournaments(year=None):
    """Convenience function to scrape PGA Tour Americas tournaments."""
    return PGAAmericasTournamentScraper().run(year=year)
