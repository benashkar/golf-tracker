"""
LPGA Tour Tournament Scraper
=============================

Scrapes LPGA Tour tournament schedules and results using ESPN API.
...
class LPGATournamentScraper(BaseScraper):
    ...
    def scrape(self, year: Optional[int] = None, **kwargs) -> Dict[str, Any]:
        """
        Scrape LPGA Tour tournaments for a given year.
    ...
    def _fetch_schedule(self, year: int) -> Optional[List[Dict]]:
        """Fetch LPGA tournament schedule from ESPN."""
    ...
    def _fetch_current_results(self):
        """Fetch results for current/in-progress LPGA events."""
        data = self.get_json(self.espn_scoreboard)
        if not data:
            return
        ...
        for event in events:
            ...
            if event_status in ['STATUS_IN_PROGRESS', 'STATUS_FINAL', 'STATUS_SCHEDULED']:
                self.logger.info(f"Processing LPGA event: {event_name} (status: {event_status})")
                self._process_event_results(event)

    def _process_event_results(self, event: Dict):
        """Process results for an ESPN LPGA event."""
        ...
        with self.db.get_session() as session:
            ...
            # Find tournament by name (fuzzy match on short name too)
            ...
            if not tournament:
                ...
                tournament = Tournament(
                    ...
                    end_date=end_date,
                    status='in_progress',
                    lpga_tournament_id=event.get('id', ''),
                )
    ...
    def _save_competitor_result(self, session, tournament: Tournament, competitor: Dict, event_status: str):
        ...
        if not player:
            if not first_name or not last_name:
                return
            player = Player(first_name=first_name, last_name=last_name, lpga_id=espn_id)
            session.add(player)
            session.flush()
        ...
        # Parse round-by-round scores from linescores
    ...
    def _parse_score(self, score_str: str) -> Optional[int]:
        """Parse score string to integer (e.g., '-13' -> -13, 'E' -> 0)."""
        if not score_str:
            return None
        if score_str == 'E':
            return 0
...
def scrape_lpga_tournaments(year=None):
    """Convenience function to scrape LPGA tournaments."""
