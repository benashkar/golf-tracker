"""
Korn Ferry Tour Tournament Scraper
===================================

Scrapes Korn Ferry Tour tournament schedules and results using the PGA Tour GraphQL API.
...
class KornFerryTournamentScraper(BasePGAEcosystemScraper):
    """Scrapes Korn Ferry Tour tournament schedules and results."""
    ...
    def scrape(self, year: Optional[int] = None, **kwargs) -> Dict[str, Any]:
        """
        Scrape Korn Ferry Tour tournaments for a given year.
        ...
        with self.db.get_session() as session:
            ...
            if tournament:
                ...
                tournament = Tournament(
                    ...
                    korn_ferry_tournament_id=data.get('tournament_id', '')
                )
    ...
    def _save_result(self, session, tournament, result, tournament_status='COMPLETED'):
        ...
        if not player:
            if not first_name or not last_name:
                return
            player = Player(first_name=first_name, last_name=last_name, korn_ferry_id=tid)
            session.add(player)
            session.flush()
...
def scrape_korn_ferry_tournaments(year=None):
    """Convenience function to scrape Korn Ferry tournaments."""
